...
export default function DecisionWorkbench(){
 ...
 const feedbackJob=async(job:DecisionJob)=>{if(brainxMode!=="connected")return notify("演示模式暂不写入反馈");const reason=window.prompt("为什么对这个职位不感兴趣？例如：方向不符、客户不熟、暂时不想做");if(!reason?.trim())return;try{await sendRecommendationFeedback(job.id,reason.trim(),brainxRun.snapshotId,makeIdempotencyKey(`recommendation-feedback:${job.id}`));setBrainxJobs(current=>current?current.filter(item=>item.id!==job.id):current);notify("已记录不感兴趣，并补充下一条候选") }catch(error){notify(`反馈失败：${error instanceof Error?error.message:"后端未响应"}`)}};
