...
export default function DecisionWorkbench(){
 ...
 useEffect(()=>{if(!hydrated)return;let cancelled=false;void(async()=>{try{await brainxFetch("/api/v1/oauth/status");const snapshot=await getSnapshot();if(!cancelled)brainxApply.current(snapshot);void loadBrainxSide.current()}catch{if(!cancelled)setBrainxMode("offline")}})();return()=>{cancelled=true}},[hydrated]);
  ...
  <main className="main">
   ...
   <div className="content">
    {detail?<JobDetail job={detail} onBack={()=>setDetail(null)} weights={weights} eventType={eventType} setEventType={setEventType} hc={hc} setHc={setHc} notify={notify}/>:clientDetail?<ClientDetail c={clientDetail} onBack={()=>setClientDetail(null)} notify={notify}/>:<>
     ...
     {page==="rules"&&<Rules weights={weights} setWeights={setWeights} notify={notify} mode={brainxMode} policy={brainxRun.policyVersion} keywords={brainxKeywords} note={brainxNote} onSaveKeywords={(k:string[],n:string)=>void(async()=>{try{await brainxFetch("/api/v1/profile",{method:"PUT",body:{profile_keywords:k,profile_note:n}});setBrainxKeywords(k);setBrainxNote(n);notify("画像已保存；下一轮推荐将生效")}catch(error){notify(`保存失败：${error instanceof Error?error.message:"后端未响应"}`)}})()}/>}
...
function DecisionToday({activeJobId,completed,jobs,engagement,sync,open,onAction,showVerification=true,tray,onToggleTray,onRemoveTray,onConfirmTray,folders,folderMode,onFolderMode,onAssignFolder,onCreateFolder}:{activeJobId:string|null;completed:string[];jobs:DecisionJob[];engagement:Record<string,EngagementState>;sync:SyncStatus;open:(job:DecisionJob,tab?:"judgement"|"engagement"|"trail"|"replay")=>void;onAction:(job:DecisionJob,action:DecisionAction)=>void;showVerification?:boolean;tray:string[];onToggleTray:(id:string)=>void;onRemoveTray:(id:string)=>void;onConfirmTray:()=>void;folders:PickFolder[];folderMode:boolean;onFolderMode:()=>void;onAssignFolder:(jobId:string,folderId:string)=>void;onCreateFolder:(name:string)=>void}){
...
function DecisionZone({tone,title,subtitle,jobs,isContext,completed,engagement,open,onAction,tray,onToggleTray,folderMode,folders,onAssignFolder}:{tone:"accepted"|"pending";title:string;subtitle:string;jobs:DecisionJob[];isContext:boolean;completed:string[];engagement:Record<string,EngagementState>;open:(job:DecisionJob,tab?:"judgement"|"engagement"|"trail"|"replay")=>void;onAction:(job:DecisionJob,action:DecisionAction)=>void;tray:string[];onToggleTray:(id:string)=>void;folderMode:boolean;folders:PickFolder[];onAssignFolder:(jobId:string,folderId:string)=>void}){return <section className={`decision-zone ${tone}${isContext?" is-context":""}`}><div className="decision-group-head"><div><span className="decision-zone-kicker">{tone==="accepted"?"TODAY'S COMMITMENTS":"NOT YET ACCEPTED"}</span><h2>{title}</h2></div><span>{isContext?`当前查看 · ${jobs.length}`:`${jobs.length} 个`}</span></div><div className="pick-grid">{jobs.map((job,index)=><DecisionCard key={job.id} job={{...job,rank:index+1}} completed={completed} engagement={engagement[job.id]||"NEW"} open={open} onAction={onAction} inTray={tray.includes(job.id)} onToggleTray={onToggleTray} folderMode={folderMode} folders={folders} onAssignFolder={onAssignFolder}/>)}</div></section>}

function DecisionCard({job,completed,engagement,open,onAction,inTray,onToggleTray,folderMode,folders,onAssignFolder}:{job:DecisionJob;completed:string[];engagement:EngagementState;open:(job:DecisionJob,tab?:"judgement"|"engagement"|"trail"|"replay")=>void;onAction:(job:DecisionJob,action:DecisionAction)=>void;inTray:boolean;onToggleTray:(id:string)=>void;folderMode:boolean;folders:PickFolder[];onAssignFolder:(jobId:string,folderId:string)=>void}){
 ...
 return <article className={`pick-card${inTray?" in-tray":""}`} onClick={()=>open(job)}>
  ...
  <div className="pick-card-foot"><small>{job.recentSignal}</small><button className={`pick-card-action${actionComplete?" complete":""}`} onClick={e=>{e.stopPropagation();onAction(job,action)}} disabled={actionComplete}>{actionComplete?"已记录":action.label}<ChevronRight/></button></div>
 </article>
}
...
function DecisionDrawer({job,tab,completed,engagement,events,outcomes,mode,legalMap,replayData,onReplay,onFactsUpdated,notify,onTab,onAction,onCommand,onOutcome}:{job:DecisionJob;tab:"judgement"|"engagement"|"trail"|"replay";completed:string[];engagement:EngagementState;events:DecisionEvent[];outcomes:Outcome[];mode:"connecting"|"connected"|"offline";legalMap:Record<string,EngagementCommand[]>;replayData?:BrainxReplay;onReplay:(jobId:string,data:BrainxReplay)=>void;onFactsUpdated:()=>Promise<void>;notify:(text:string)=>void;onTab:(tab:"judgement"|"engagement"|"trail"|"replay")=>void;onAction:(job:DecisionJob,action:DecisionAction)=>void;onCommand:(job:DecisionJob,command:EngagementCommand)=>void;onOutcome:(job:DecisionJob,stage:Outcome["stage"],rating?:number,note?:string)=>void}){
 ...
 useEffect(()=>{if(mode!=="connected"||!job?.brainxDecisionId||replayData)return;let cancelled=false;setReplayLoading(true);brainxFetch(`/api/v1/decisions/${encodeURIComponent(job.brainxDecisionId)}/replay`).then(data=>{if(!cancelled)onReplay(job.id,mapReplayData(data))}).catch(()=>{}).finally(()=>{if(!cancelled)setReplayLoading(false)});return()=>{cancelled=true}},[mode,job?.id,job?.brainxDecisionId,replayData,onReplay]);
...
function JobsView({jobs,status,setStatus,sort,setSort,view,setView,selected,setSelected,openJob,notify,mode}:any){
 ...
 const clientOptions:FilterSelectOption[]=[{value:"全部客户",label:"全部客户"},...Array.from(new Set(jobs.map((job:Job)=>job.client))).sort().map(value=>({value,label:value}))];
 const cityOptions:FilterSelectOption[]=[{value:"全部城市",label:"全部城市"},...Array.from(new Set(jobs.map((job:Job)=>job.city))).sort().map(value=>({value,label:value}))];
